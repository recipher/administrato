...
export const handle = {
  ...
  breadcrumb: ({ legalEntity, current, name }: { legalEntity: any } & BreadcrumbProps) => 
    [ <Breadcrumb to={`/schedules/legal-entities/${legalEntity?.id}/generate`} name={name.at(0) as string} current={current} />,
      <Breadcrumb to={`/schedules/legal-entities/${legalEntity?.id}/generate/approvers`} name={name.at(1) as string} current={current} /> ]
};
