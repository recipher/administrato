...
export const handle = {
  ...
  breadcrumb: ({ client, current, name }: { client: Client } & BreadcrumbProps) => 
    <Breadcrumb to={`/manage/client/${client.id}/legal-entities/add`} name={name} current={current} />
};
...
export const loader = async ({ params, request }: LoaderArgs) => {
  ...
  const securityGroup = securityGroupId ? await service.getSecurityGroup({ id: securityGroupId }) : undefined;
  ...
  return json({ client, securityGroup, frequencies, targets, weekdays });
};
...
export const action = async ({ params, request }: ActionArgs) => {
  ...
  return redirect(`/manage/clients/${id}/legal-entities`);
};
  ...
  return (
    <>
      <Form method="POST" defaultValues={{ targets: [ { target: { id: "last" }}]}}
        validator={clientValidator} id="add-legal-entity" encType="multipart/form-data" className="mt-6">
        <Body>
