...
export const action = async ({ request, params }: ActionArgs) => {
  ...
  await service.addAddress(create({ country, countryIsoCode,
    classifier: 'business', entityId: id, entity: 'client', 
...
const Add = () => {
  ...
  const [ country, setCountry ] = useState(countryData.find((c: any) => c.id === client.localities.at(0) ));
  ...
  const changeAddressFormat = ({ country }: any) => {
    ...
    // @ts-ignore
    address.setFormat({ country: country.id, type: 'business', useTransforms: false });
    setAddressConfig(address.toArray());
  };

  useEffect(() => {
    if (fetcher.state === 'idle') fetcher.load(`/countries/${country.id}/regions`);
    changeAddressFormat({ country });
  }, [ country ]);
  ...
  return (
    <>
      <Form method="POST" validator={validator} id="add-address" encType="multipart/form-data" className="mt-6">
        ...
        <Footer>
          <Cancel />
          <Submit text="Save" submitting="Saving..." permission={manage.edit.client} />
        </Footer>
      </Form>
    </>
  );
